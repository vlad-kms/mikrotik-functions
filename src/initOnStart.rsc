...
##############################
  ...
  do {
    :local res [/tool e-mail send from=$acct to=$to server=[:resolve $serv]  tls=$lTLS port=$portL user=$acct password=$passwd subject=$subj body=$body as-value cc=$cc]
  } on-error={
    :if ($isDeb) do={
...
##############################
  ...
  :if ( $islogFunc ) do={
    :log info ("function statusIF (".$ifname.") START ===========================");
  }
